    ...
    <div className="fixed top-0 left-0 w-full z-50">
      ...
      <div className="bg-red-600 text-white py-2 text-sm md:text-lg font-semibold overflow-hidden">
        <div className="flex whitespace-nowrap animate-scroll">
          <span className="mx-8 md:mx-16">🎨 Get 20% OFF on your first order!</span>
          <span className="mx-8 md:mx-16">🖌️ New Arrivals Just Dropped!</span>
          <span className="mx-8 md:mx-16">🚚 Free Shipping on orders above ₹5000</span>
          <span className="mx-8 md:mx-16">🎁 Custom Art Commissions Available – Contact us now!</span>
        </div>
      </div>
        ...
        <div className="flex items-center space-x-4 md:space-x-6 relative">
          ...
          <div className="relative">
            <button onClick={openDropdown} className="focus:outline-none">
              <User className="h-6 w-6 md:h-8 md:w-8 text-gray-800 hover:text-red-600" />
            </button>
            {isDropdownOpen && (
          ...
          <Link to="/cart" className="relative">
            <ShoppingCart className="h-6 w-6 md:h-8 md:w-8 text-gray-800 hover:text-red-600" />
            {cart.length > 0 && (
      ...
      {/* Search Bar */}
      {isSearchBarOpen && (
        <div className="w-full bg-white border-t border-b shadow-md py-4 px-4 md:px-20 flex flex-col md:flex-row items-start md:items-center gap-4">
          <form
